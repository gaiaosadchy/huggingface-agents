# Hugging Face Agents Course

This repository documents my progress through the Hugging Face Agents Course. It contains scripts, experiments, and tools built using the `smolagents` library.

## Current Status
- Local LLM Testing (Ollama + Qwen2 7B)

## Project Structure
* `check_ollama.py`: A verification script to test the connection between `smolagents` and local/remote LLM servers.
* `.gitignore`: Configured to exclude heavy `.venv` and `__pycache__` files.
* `README.md`: You are here!
